...
public interface ProdutoRepository extends JpaRepository<Produto, Integer>, CrudRepository<Produto, Integer> {
	...
	@Query(value = "select * from produto p where p.id_categoria != 5  order by p.id_produto desc limit 8", nativeQuery = true)
	List<Produto> findNovidades();
	
	@Query(value = "select * from produto p   where p.id_categoria != 5 order by  p.preco  asc limit 8", nativeQuery = true)
	...
	@Query(value = " select * from produto where id_categoria = :id ", nativeQuery = true)
	List<Produto> idcategoria(@Param("id") String id);
